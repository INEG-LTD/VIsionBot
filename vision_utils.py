...
def validate_bounding_box(box: List[int]) -> List[int]:
    """
    Validate and fix a bounding box to ensure it has valid coordinates.
    ...
    """
    if len(box) != 4:
        raise ValueError("Bounding box must have 4 values: [x_min, y_min, x_max, y_max]")
    
    # Clamp and validate coordinates
    clamped_box = []
    for coord in box:
        if not isinstance(coord, (int, float)) or not math.isfinite(coord):
            clamped_box.append(0)  # Safe fallback
        else:
            clamped_box.append(max(0, int(coord)))  # Clamp to >= 0
    
    x_min, y_min, x_max, y_max = clamped_box
    
    # Ensure min < max (add 1px if they're equal)
    if x_min >= x_max:
        x_max = x_min + 1
    if y_min >= y_max:
        y_max = y_min + 1
...
def validate_gemini_box_2d(box: List[int]) -> List[int]:
    """
    Validate and fix a Gemini box_2d to ensure it has valid coordinates.
    
    Args:
        box: Gemini box_2d as [y_min, x_min, y_max, x_max] normalized 0-1000
    ...
    """
    if len(box) != 4:
        raise ValueError("Gemini box_2d must have 4 values: [y_min, x_min, y_max, x_max]")
    
    # Clamp and validate coordinates to 0-1000 range
    clamped_box = []
    for coord in box:
        if not isinstance(coord, (int, float)) or not math.isfinite(coord):
            clamped_box.append(0)  # Safe fallback
        else:
            clamped_box.append(max(0, min(1000, int(coord))))  # Clamp to 0-1000
    
    y_min, x_min, y_max, x_max = clamped_box
    
    # Ensure min < max (add 1 unit if they're equal)
    if y_min >= y_max:
        y_max = y_min + 1
    if x_min >= x_max:
        x_max = x_min + 1
...
def get_bounding_box_center(box: List[int]) -> Tuple[int, int]:
    """
...
def is_coordinate_valid(x: int, y: int, page_width: int, page_height: int) -> bool:
    """
    Check if coordinates are valid within the page bounds.
    ...
    """
    return (isinstance(x, (int, float)) and math.isfinite(x) and
            isinstance(y, (int, float)) and math.isfinite(y) and
