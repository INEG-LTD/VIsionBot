"""Cost tracking middleware for Browser."""

from middleware.base import Middleware, ActionContext, CostLimitExceeded
    ...
    def after_action(self, context: ActionContext, result: Any) -> Any:
        ...
        if isinstance(result, dict) and 'cost' in result:
            ...
            if not self.warned and self.total_cost >= (self.max_cost * self.warn_at):
                ...
                get_event_logger().cost_warning(self.total_cost, self.max_cost)
                self.warned = True
            
            # Check if we exceeded limit
            if self.total_cost > self.max_cost:
                get_event_logger().cost_limit_exceeded(self.total_cost, self.max_cost)
