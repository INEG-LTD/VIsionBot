...
class VisionPlan(BaseModel):
    ...
    detected_elements: List[DetectedElement] = Field(description="All detected UI elements")
...
class VisionBot:
    ...
    def achieve_goal(self, goal_description: str, additional_context: str = "", smart_goals: bool = True) -> bool:
        ...
        for attempt in range(self.max_attempts):
            ...
            if plan and plan.detected_elements:
                draw_bounding_boxes(screenshot, plan.detected_elements, save_path="image_with_bounding_boxes.png")
            ...
            if success:
                ...
                # Traditional goal checking disabled for now
                # elif self._is_goal_achieved(goal):
                #     print("✅ Goal achieved after executing plan!")
                #     return True
    ...
    def _setup_smart_goals(self, goal_description: str, additional_context: str = "") -> None:
        ...
        click_patterns = [
            ...
            "click on", "click the", "press the", "tap the"
        ]
        ...
        # - NavigationGoal for "go to", "navigate to"
    ...
    def _print_goal_summary(self) -> None:
        ...
        print(f"   ⏱️  Duration: {summary['session_duration']:.1f}s")
    ...
    def _generate_plan(
        ...
        """Generate an action plan using the vision model"""
        
        system_prompt = f"""
        You are a web automation assistant. Look at the screenshot and create a plan to achieve the user's goal.
        ...
        Instructions:
        1. Analyze the screenshot to identify relevant UI elements
        2. Focus on elements that help achieve the goal (forms, buttons, links, etc.)
        3. Create a plan with 1-3 action steps
        4. Use Gemini box_2d format: [y_min, x_min, y_max, x_max] normalized to 0-1000
        5. Be conservative - don't click destructive buttons unless specifically asked
        
        Return a VisionPlan with:
        - detected_elements: List of relevant UI elements with box_2d coordinates
        - action_steps: Specific steps to take (keep it simple!)
        ...
        user_prompt = "Analyze this screenshot and create a plan to achieve the goal. Return only the JSON for VisionPlan."
        ...
        try:
            ...
            # Validate and clamp all coordinates in the plan
            if plan:
                plan = self._validate_plan_coordinates(plan, page_info)
    ...
    def _validate_plan_coordinates(self, plan: VisionPlan, page_info: PageInfo) -> VisionPlan:
        ...
        try:
            ...
            for element in plan.detected_elements:
                if element.box_2d:
            ...
            print(f"⚠️ Error validating plan coordinates: {e}")
    ...
    def _execute_click(self, step: ActionStep, elements: List[DetectedElement], page_info: PageInfo):
        ...
        if any(result.status == GoalStatus.ACHIEVED for result in pre_evaluations.values()):
            print("🎯 Goal achieved before click! Proceeding with click to complete the interaction.")
    ...
    def _execute_type(self, step: ActionStep, elements: List[DetectedElement], page_info: PageInfo):
    ...
    def _get_click_coordinates(self, step: ActionStep, elements: List[DetectedElement], page_info: PageInfo) -> Tuple[Optional[int], Optional[int]]:
        ...
        if step.target_element_index is not None:
            if 0 <= step.target_element_index < len(elements):
                element = elements[step.target_element_index]
                if element.box_2d:
    ...
    def _is_goal_achieved(self, goal: Goal) -> bool:
        ...
        try:
            ...
            print(f"⚠️ Error checking goal achievement: {e}")
            return False
