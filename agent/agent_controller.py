...
from core.config import Config, SequentialTaskConfig, CompletionStrategy
...
class Agent:
    ...
    def __init__(
        ...
        self._temp_end_sequence: bool = False
        self._temp_end_sequence_reasoning: Optional[str] = None
    ...
    def _run_mission(self, user_mission: str) -> MissionResult:
        ...
        self.task_list = MissionPlan(
            tasks=[
                Sequence(
                    target_count=3,
                    extraction_schema={},
                    task="type hello 3 times",
                    task_id="task_1",
    ...
    def _decompose_user_mission_into_tasks(
        ...
        try:
            ...
            for i, task in enumerate(task_list.tasks, 1):
                ...
                self.event_logger.system_debug(f"  Task {i} ({task_type}): {task.task}")
        except Exception:
            pass
    ...
    def _execute_task_list(
        ...
        while self.task_list.get_current_task() is not None:
            current_task: Task | Sequence = self.task_list.get_current_task()
            ...
            if isinstance(current_task, Task):
                result = self._execute_normal_task(current_task, user_mission)
            elif isinstance(current_task, Sequence):
                result = self._execute_sequential_task(current_task, user_mission)
            ...
            if result.success:
                ...
                task_type = "Normal" if isinstance(current_task, Task) else "Sequential"
                self.event_logger.task_complete(current_task.task_id, task_type=task_type)
                ...
                current_task.completed_at = time.time()
                task_type = "Normal" if isinstance(current_task, Task) else "Sequential"
                self.event_logger.task_fail(current_task.task_id, error=result.reasoning, task_type=task_type)
                
                # When a task fails, we need to end the mission
        ...
        if len(completed_tasks) > 0:
            ...
            # mission_result.reasoning = completed_tasks[len(completed_tasks) - 1].reasoning
        else:
            mission_result.success = False
    ...
    def _execute_normal_task(
        ...
        result = self.run_task_loop(
            ...
            original_prompt=user_mission,
            extraction_schema=getattr(task, 'extraction_schema', None),
            current_sequence_task=current_sequence_task,
        )

        if not result.success:
            self.event_logger.task_fail(task.task_id, error=result.reasoning)
    ...
    def _execute_sequential_task(
        ...
        Args:
            sequential_task: The sequential task to execute
            user_mission: Original user mission (for context)

        Returns:
            True if task completed successfully (based on completion strategy)
        """
        sequential_task.status = TaskStatus.IN_PROGRESS

        self.event_logger.sequential_task_start(sequential_task)
        task_format = self.sequence_planner.decide_sequence_task_format(sequential_task) or ""

        # Sequential execution loop
        while True:
            should_end, reason = self.sequence_planner.should_end_sequence(sequential_task)
            print(f"should_end: {should_end}, reason: {reason}, _temp_end_sequence: {self._temp_end_sequence}")
            ...
            task = f"{task_format}"
            self.event_logger.sequence_decision(task)

            # Handle decision
            if self._temp_end_sequence:
                self._temp_end_sequence = False
                self._temp_end_sequence_reasoning = None
                self._current_sequential_iteration += 1
                print(f"current_sequential_iteration: {self._current_sequential_iteration}")

            # Execute the generated task
            generated_task_instruction = task
            sequential_task.current_subtask = generated_task_instruction
            self.event_logger.subtask_start(generated_task_instruction)
            subtask_task_id = f"{sequential_task.task_id}-{sequential_task.state.current_turn}"
            # Execute the generated task (using existing ActionPlanner)
            result: TaskResult = self._execute_normal_task(
                task=Task(
                    task_id=subtask_task_id,
                    goal=generated_task_instruction,
                    created_at=time.time(),
                    completed_at=None,
                    status=TaskStatus.IN_PROGRESS,
                ),
                user_mission=user_mission,
                current_sequence_task=sequential_task.task,
            )

            # Handle result
            if result.success:
                self.event_logger.subtask_complete(generated_task_instruction)
                # Record successful iteration
                self._record_subtask_success(subtask_task_id, sequential_task, result)

            else:
                # Max retries reached, record failure and move to next iteration
                self._record_subtask_failure(subtask_task_id, sequential_task, result)
                self.event_logger.subtask_fail(generated_task_instruction, error=result.reasoning)

            # Move to next iteration
            sequential_task.state.current_turn += 1
                
        self.event_logger.sequential_task_complete(sequential_task)

        # Determine success based on completion strategy
        cfg = self.sequential_task_config
        strategy = cfg.completion_strategy

        success = False
        reasoning = None
        if strategy == CompletionStrategy.BEST_EFFORT:
            success = True
            reasoning = "Sequential task completed: best effort"

        if strategy == CompletionStrategy.STRICT:
            # If target_count is set, require >= target successes.
            # If indefinite, succeed if we got at least one success.
            target = sequential_task.target_count
            success = (sequential_task.state.total_success_count >= target) if target != -1 else (sequential_task.state.total_success_count > 0)
            reasoning = f"Sequential task completed: {sequential_task.state.total_success_count} / {target}"
            if not success:
                target_display = sequential_task.target_count if sequential_task.target_count != -1 else "indefinite"
                self.event_logger.sequential_task_fail(
                    sequential_task, 
                    error=f"Target not reached: {sequential_task.state.total_success_count} / {target_display}"
                )
                reasoning = f"Target not reached: {sequential_task.state.total_success_count} / {target_display}"

        # THRESHOLD
        target = sequential_task.target_count
        total = sequential_task.state.total_success_count + sequential_task.state.total_failure_count
        if target == -1:
            # For indefinite sequences, use success rate of all attempts
            if total == 0:
                self.event_logger.sequential_task_fail(sequential_task, error="No attempts made (indefinite sequence)")
                reasoning = "No attempts made (indefinite sequence)"
                success = False
            success_rate = sequential_task.state.total_success_count / total
            success = success_rate >= cfg.success_threshold
            if not success:
                self.event_logger.sequential_task_fail(
                    sequential_task, 
                    error=f"Success rate below threshold: {success_rate:.1%} < {cfg.success_threshold:.1%}"
                )
                reasoning = f"Success rate below threshold: {success_rate:.1%} < {cfg.success_threshold:.1%}"

        
        if total == 0:
            self.event_logger.sequential_task_fail(sequential_task, error="No attempts made (finite sequence)")
            reasoning = "No attempts made (finite sequence)"
            success = False
            return TaskResult(success=success, reasoning=reasoning)

        success_rate = sequential_task.state.total_success_count / total
        success = success_rate >= cfg.success_threshold
        reasoning = f"Sequential task completed: {success_rate:.1%} >= {cfg.success_threshold:.1%}"
    
        if not success:
            self.event_logger.sequential_task_fail(sequential_task, error=f"Success rate below threshold: {success_rate:.1%} < {cfg.success_threshold:.1%}")
            reasoning = f"Success rate below threshold: {success_rate:.1%} < {cfg.success_threshold:.1%}"
        return TaskResult(success=success, reasoning=reasoning)
    ...
    def _record_subtask_success(
        ...
        """
        Record a successful subtask in sequential task.
        ...
        if result.evidence and "extracted_data" in result.evidence:
            extracted_data = result.evidence.get("extracted_data", {})
            
        # Create subtask result
        sub_task_result = SequenceSubTaskResult(
            turn=turn_idx,
            status="success",
            result=extracted_data,
            task_attempted=sequential_task.current_subtask,
        )

        # Add to completed subtasks
        sequential_task.state.completed_turns.append(sub_task_result)
        sequential_task.state.total_success_count += 1

        self.notebook.add_subtask_result(
            task_id=task_id,
            description=sequential_task.current_subtask,
            data=extracted_data,
            entry_type=NotebookEntryType.SEQUENTIAL_SUBTASK_RESULT,
            subtask_turn=turn_idx,
        )
    ...
    def _record_subtask_failure(
        ...
        """
        Record a failed subtask in sequential task.
